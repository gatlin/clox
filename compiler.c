...
advance () {
  ...
  for (;;) {
    ...
    if (TOKEN_ERROR != parser.current.type) { break; }
    errorAtCurrent (parser.current.start);
  }
}

static void
expect (TokenType type, const char *message) {
...
#ifdef  DEBUG_PRINT_CODE
  if (!parser.hadError) {
    disassembleChunk (currentChunk (), NULL != function->name
      ? function->name->chars
      : "<script>");
  }
#endif
...
block () {
  ...
  expect (TOKEN_RIGHT_BRACE, "Expect '}' after block.");
}
...
static void
call (bool _canAssign) {
  uint8_t argCount = argumentList ();
  emitBytes (OP_CALL, argCount);
}

static void
literal (bool _canAssign) {
...
static void
grouping (bool _canAssign) {
  expression ();
  expect (TOKEN_RIGHT_PAREN, "Expect ')' after expression.");
}

static void
number (bool _canAssign) {
  double value = strtod (parser.previous.start, NULL);
  emitConstant (NUMBER_VAL(value));
}

static void
and_ (bool _canAssign) {
...
addUpvalue (Compiler *compiler, uint8_t index, bool isLocal) {
  ...
  for (; i < upvalueCount; i++) {
    upvalue = &compiler->upvalues[i];
    if (isLocal == upvalue->isLocal && index == upvalue->index) {
      return i;
    }
  }
...
static void
binary (bool _canAssign) {
  TokenType operatorType = parser.previous.type;
  Precedence precedence = getRule (operatorType)->precedence;
...
static void
unary (bool _canAssign) {
  TokenType operatorType = parser.previous.type;
  parsePrecedence (PREC_UNARY);
  switch (operatorType) { /* Emit operator instruction. */
...
parsePrecedence (Precedence precedence) {
  ...
  ParseFn rule = getRule (parser.current.type)->prefix;
  if (NULL == rule) {
  ...
  while (precedence <= getRule (parser.current.type)->precedence) {
    rule = getRule (parser.current.type)->infix;
    advance ();
    rule (canAssign);
  }
  if (canAssign && match (TOKEN_EQUAL)) {
...
static void
forStatement () {
  int loopStart, bodyJump, incrementStart,  exitJump = -1;
  beginScope ();
  expect (TOKEN_LEFT_PAREN, "Expect '(' after 'for'.");
  ...
  if (!match (TOKEN_RIGHT_PAREN)) {
    ...
    expression ();
    emitByte (OP_POP);
    expect (TOKEN_RIGHT_PAREN, "Expect ')' after 'for' clauses.");
    emitLoop (loopStart);
...
whileStatement () {
  ...
  expression ();
  expect (TOKEN_RIGHT_PAREN, "Expect ')' after condition.");
  exitJump = emitJump (OP_JUMP_IF_FALSE);
  emitByte (OP_POP);
  statement ();
  emitLoop (loopStart);
